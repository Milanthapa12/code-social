...
import { Notification as NotifactionData } from '@/lib/type'
...
interface INotifcationProps {
    notification: NotifactionData
}

export default function Notification({ notification }: INotifcationProps) {

    const notificationTypeMap: Record<NotificationType, { message: string, icon: JSX.Element, href: string }> = {
        FOLLOW: {
            message: `${notification.issuer.name} followed you`,
        ...
        <Link href={href} className='block'>
            <article className={cn("flex gap-3 rounded-2xl bg-card p-5 shadow-sm transition-colors hover:bg-card/70", !notification.read && "bg-primary/10")}>
                ...
                <div className="space-y-3">
                    <UserAvatar avatarURL={notification.issuer.avatar} size={36} />
                    <div>
